...
def create_index(tokens):
    ...
    for l in range (len(tokens)):
    	for t in tokens[l]:
                if inverted.has_key(t):
                        if l not in inverted[t]:
                                inverted[t].append(l)
                else:
                        inverted.update({t:[l]})
...
def search(index, query):
    ...
    [1]
    """
    q_words = sort_by_num_postings(tokenize(query), index)
    tmp = index[q_words[0]]
    ...
    for q in q_words[1:]:
	    ...
	    tmp = intersect(tmp, r)
    """if (len(query)) == 2:
    	tmp = intersect(tmp, index[qtokens[1]])	
    else:
	    while i < len(query)-1:
		    tmp = intersect(tmp, index[qtokens[i]])
		    i+=1"""
